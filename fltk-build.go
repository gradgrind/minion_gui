...
func main() {
	...
	fmt.Fprintln(cgoFile, "package minion_gui\n")
	fmt.Fprintf(cgoFile, "// #cgo %s,%s CXXFLAGS: -std=c++20\n", runtime.GOOS, runtime.GOARCH)
	...
	if runtime.GOOS != "windows" {
		...
		fltkConfigCxxCmd := exec.Command(fltkConfigPath, "--use-gl", "--use-images", "--cxxflags")
		...
		fltkConfigLdCmd := exec.Command(fltkConfigPath, "--use-gl", "--use-images", "--ldstaticflags")
